...
class CompletedJobsController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('job.completed_jobs', $data);
    }

    public function get_customer_list(Request $request)
    {
        $completed_jobs = \App\Model\CompletedJobs::select('id', 'inquiry_id', 'completed_date', 'invoice_value')
                ->with(['Inquiry' => function ($query) {
                    $query->select('id', 'contact_id', 'sales_team_id')
                ...
                ->whereBetween('completed_date', [$request->from, $request->to])
                ->get();

        return response($completed_jobs);
    }

    public function get_completed_job_details(Request $request)
    {
        $data['main_menus'] = \App\Model\UserAccess::leftJoin('side_menu', 'side_menu.id', '=', 'user_access.side_menu_id')
                ->whereIn('user_access.user_group_id', session()->get('user_group'))
                ->where('side_menu.menu_category', 0)
        ...
        $data['inquiry'] = \App\Model\Inquiry::find($request->id);

        return view('job.completed_job_details', $data);
    }
}
