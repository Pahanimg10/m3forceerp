        ...
        <form autocomplete="off" id="dataForm" name="dataForm" class="form-horizontal" j-validate>
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3 class="panel-title"><strong>Region</strong></h3>
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <div class="col-md-10">
                                            <input type="text" id="name" name="name" placeholder="Colombo 02" ng-model="data.name" class="form-control" required="" />
                                        </div>
                                    </div>
                                </div>
...
<script type="text/javascript">
    ...
    myApp.controller('menuController', function ($scope) {
        ...
        angular.element(document.querySelector('#sub_menu_region')).addClass('active');
    });   
                ...
                element.validate({
                    rules: {
                        name: {
                            required: true,
                            remote: {
                                url: base_url + '/region/validate_region',
                                type: 'GET',
                                data: {
                                    value: function() {
    ...
    myApp.controller('mainController', ['$scope', '$http', '$rootScope', 'uiGridConstants', 'uiGridExporterConstants', '$timeout', '$q', '$window', function ($scope, $http, $rootScope, uiGridConstants, uiGridExporterConstants, $timeout, $q, $window) {  
            ...
            $scope.gridOptions = {
                ...
                exporterCsvFilename: 'Regions.csv',
            ...
            $scope.editRecord = function (row) {
                ...
                $http({
                    method: 'GET',
                    url: base_url + '/region/find_region',
                    params: {
                        id: row.entity.id
                    }
            ...
            $scope.deleteRecord = function (row) {
                ...
                $http({
                    ...
                    swal({
                        title: "Are you sure?",
                        text: "You will not be able to recover <strong>" + response.data.name + "</strong> region!",
                        html: true,
                        type: "warning",
                    ...
                    function(){
                        $http.delete(base_url + '/region/'+$scope.data.id, $scope.data).success(function (response_delete) {
                            swal({
                                title: "Deleted!", 
                                text: response.data.name + " region has been deleted.", 
                                type: "success",
            ...
            submitForm = function(){
                ...
                if($scope.data.id == 0){
                    $http.post(base_url + '/region', $scope.data).success(function (result) {
                    ...
                    $http.put(base_url + '/region/'+$scope.data.id, $scope.data).success(function (result) {
                        $('#save_button').prop('disabled', false);
                        $.pnotify && $.pnotify({
                            title: 'Regions',
                            text: result.message,
            ...
            $scope.main_refresh = function(){
                ...
                $http({
                    method: 'GET',
                    url: base_url + '/region/region_list'
                }).then(function successCallback(response) {
                    var data_array = [];
