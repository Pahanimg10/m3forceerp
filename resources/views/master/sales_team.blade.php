...
@section('title')
<title>M3Force | Sales Team</title>
@endsection
        ...
        <form autocomplete="off" id="dataForm" name="dataForm" class="form-horizontal" j-validate>
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3 class="panel-title"><strong>Sales Team</strong></h3>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="row">
                                ...
                                <div class="col-md-3">
                                    <div class="form-group">
                                        <label class="col-md-4 control-label">Sales Target</label>             
                                        <div class="col-md-8">
                                            <input type="text" id="sales_target" name="sales_target" placeholder="3000000" ng-model="data.sales_target" class="form-control text-right" required="" />
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-2">
...
<script type="text/javascript">
    ...
    myApp.controller('menuController', function ($scope) {
        ...
        angular.element(document.querySelector('#sub_menu_sales_team')).addClass('active');
    });   
                ...
                element.validate({
                    rules: {
                        name: {
                            required: true,
                            remote: {
                                url: base_url + '/sales_team/validate_sales_team',
                                type: 'GET',
                                data: {
                                    value: function() {
    ...
    myApp.controller('mainController', ['$scope', '$http', '$rootScope', 'uiGridConstants', 'uiGridExporterConstants', '$timeout', '$q', '$window', function ($scope, $http, $rootScope, uiGridConstants, uiGridExporterConstants, $timeout, $q, $window) {  
            ...
            $scope.gridOptions = {
                columnDefs: [
                    ...
                    {
                        field: 'sales_target', 
                        displayName: 'Sales Target', 
                        cellClass: 'grid-align-right',
                        width: '15%', 
                        footerCellTemplate: '<div class="ui-grid-cell-contents text-right" ><%= grid.appScope.getAggregationSalesTarget() | number:2 %></div>',
                        enableCellEdit: false
                    },
                    {
                        field: 'is_active', 
                ...
                exporterCsvFilename: 'Sales Team.csv',
            ...
            $scope.getAggregationSalesTarget = function(){
                var total_value = 0;
                for(var i=0; i<$scope.gridOptions.data.length; i++){
                    total_value += Number($scope.gridOptions.data[i].sales_target);
                }
                return total_value;
            };
            ...
            $scope.editRecord = function (row) {
                ...
                $http({
                    method: 'GET',
                    url: base_url + '/sales_team/find_sales_team',
                    params: {
                        id: row.entity.id
                    }
                    ...
                    $scope.data = {
                        ...
                        sales_target: parseFloat(Math.round(response.data.sales_target * 100) / 100).toFixed(2),
            ...
            $scope.deleteRecord = function (row) {
                ...
                $http({
                    ...
                    swal({
                        title: "Are you sure?",
                        text: "You will not be able to recover <strong>" + response.data.name + "</strong> sales team!",
                        html: true,
                        type: "warning",
                    ...
                    function(){
                        $http.delete(base_url + '/sales_team/'+$scope.data.id, $scope.data).success(function (response_delete) {
                            swal({
                                title: "Deleted!", 
                                text: response.data.name + " sales team has been deleted.", 
                                type: "success",
            ...
            submitForm = function(){
                ...
                if($scope.data.id == 0){
                    $http.post(base_url + '/sales_team', $scope.data).success(function (result) {
                    ...
                    $http.put(base_url + '/sales_team/'+$scope.data.id, $scope.data).success(function (result) {
                        $('#save_button').prop('disabled', false);
                        $.pnotify && $.pnotify({
                            title: 'Sales Team',
                            text: result.message,
            ...
            $scope.main_refresh = function(){
                ...
                $http({
                    method: 'GET',
                    url: base_url + '/sales_team/sales_team_list'
                }).then(function successCallback(response) {
                    var data_array = [];
                    $.each(response.data.sales_teams, function (index, value) {
                        data_array.push({
                            id: value.id,
                            ...
                            sales_target: parseFloat(Math.round(value.sales_target * 100) / 100).toFixed(2),
