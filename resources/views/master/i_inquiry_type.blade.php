...
@section('title')
<title>M3Force | I-Inquiry Types</title>
@endsection

@section('breadcrumb')
<ul class="breadcrumb">
    <li><a href="#">Master</a></li>                    
    <li class="active">I-Inquiry Types</li>
</ul>
@endsection
        ...
        <form autocomplete="off" id="dataForm" name="dataForm" class="form-horizontal" j-validate>
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3 class="panel-title"><strong>I-Inquiry Types</strong></h3>
...
<script type="text/javascript">
    ...
    myApp.controller('menuController', function ($scope) {
        ...
        angular.element(document.querySelector('#sub_menu_i_inquiry_type')).addClass('active');
    });   
                ...
                element.validate({
                    rules: {
                        name: {
                            required: true,
                            remote: {
                                url: base_url + '/i_inquiry_type/validate_i_inquiry_type',
                                type: 'GET',
                                data: {
                                    value: function() {
    ...
    myApp.controller('mainController', ['$scope', '$http', '$rootScope', 'uiGridConstants', 'uiGridExporterConstants', '$timeout', '$q', '$window', function ($scope, $http, $rootScope, uiGridConstants, uiGridExporterConstants, $timeout, $q, $window) {  
            ...
            $scope.gridOptions = {
                ...
                exporterCsvFilename: 'I-Inquiry Types.csv',
            ...
            $scope.editRecord = function (row) {
                ...
                $http({
                    method: 'GET',
                    url: base_url + '/i_inquiry_type/find_i_inquiry_type',
                    params: {
                        id: row.entity.id
                    }
            ...
            $scope.deleteRecord = function (row) {
                ...
                $http({
                    ...
                    swal({
                        title: "Are you sure?",
                        text: "You will not be able to recover <strong>" + response.data.name + "</strong> i-inquiry type!",
                        html: true,
                        type: "warning",
                    ...
                    function(){
                        $http.delete(base_url + '/i_inquiry_type/'+$scope.data.id, $scope.data).success(function (response_delete) {
                            swal({
                                title: "Deleted!", 
                                text: response.data.name + " i-inquiry type has been deleted.", 
                                type: "success",
            ...
            submitForm = function(){
                ...
                if($scope.data.id == 0){
                    $http.post(base_url + '/i_inquiry_type', $scope.data).success(function (result) {
                        $('#save_button').prop('disabled', false);
                        $.pnotify && $.pnotify({
                            title: 'I-Inquiry Types',
                            text: result.message,
                    ...
                    $http.put(base_url + '/i_inquiry_type/'+$scope.data.id, $scope.data).success(function (result) {
                        $('#save_button').prop('disabled', false);
                        $.pnotify && $.pnotify({
                            title: 'I-Inquiry Types',
                            text: result.message,
            ...
            $scope.main_refresh = function(){
                ...
                $http({
                    method: 'GET',
                    url: base_url + '/i_inquiry_type/i_inquiry_type_list'
                }).then(function successCallback(response) {
                    var data_array = [];
                    $.each(response.data.i_inquiry_types, function (index, value) {
                        data_array.push({
                            id: value.id,
