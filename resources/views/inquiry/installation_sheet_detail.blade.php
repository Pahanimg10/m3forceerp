...
@section('title')
<title>M3Force | Installation Sheet Details</title>
@endsection

@section('breadcrumb')
<ul class="breadcrumb">
    <li><a href="#">Inquiry</a></li>                    
    <li class="active">Installation Sheet Details</li>
</ul>
@endsection
        ...
        <form autocomplete="off" id="dataForm" name="dataForm" class="form-horizontal" j-validate>
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3 class="panel-title"><strong>Installation Sheet Details</strong></h3>
                    <ul class="panel-controls">
                        ...
                        <li><button type="button" ng-show="data.installation_sheet_id && data.is_posted == 0" ng-disabled="edit_disable" ng-click="postForm()" class="btn btn-danger" id="post_button">Post</button></li>
                        <li><button type="button" ng-show="data.installation_sheet_id && data.is_posted == 1" ng-disabled="permission || data.is_approved == 1" ng-click="authorizeData()" class="btn btn-danger" data-toggle="modal" data-target="#dataModal">Authorize</button></li>
                        ...
                        <li><div><a ng-show="data.installation_sheet_id" target="_blank" class="btn btn-default" ng-href="<%=base_url%>/installation_sheet/print_installation_sheet?id=<%=data.installation_sheet_id%>">Print</a></div></li>
                        ...
                        <li><a href="{{ asset('inquiry/installation_sheet?id='.$inquiry_id.'&type='.$type)}}"><span class="fa fa-times"></span></a></li>
                    </ul>
                </div>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="row">
                                ...
                                <input type="hidden" id="installation_sheet_id" name="installation_sheet_id" ng-model="data.installation_sheet_id" class="form-control" />
                                ...
                                <div class="row">
                                    <h4 class="col-md-12" style="padding-top: 15px;">Installation Sheet Details</h4>
                                </div>
                                ...
                                <div class="row">
                                    <div class="col-md-2">
                                        <div class="form-group">
                                            ...
                                            <input type="text" id="installation_sheet_no" name="installation_sheet_no" ng-model="data.installation_sheet_no" class="form-control text-center" disabled />
                                        </div>
                                    </div>
                                    <div class="col-md-2">
                                        <div class="form-group">
                                            <label class="control-label">Installation Sheet Date</label>
                                            <input type="text" id="installation_sheet_date" name="installation_sheet_date" uib-datepicker-popup="<%=dateFormat%>" ng-model="data.installation_sheet_date" ng-disabled="edit_disable" is-open="installationSheetDatePopup.opened" datepicker-options="availableDateOptions" close-text="Close" ng-click="OpenInstallationSheetDate()" class="form-control text-center" />
                                        </div>
                                    </div>
                                    <div class="col-md-2">
                                        <div class="form-group">
                                            <label class="control-label">Installation Sheet Time</label>
                                            <input type="text" id="installation_sheet_time" name="installation_sheet_time" ng-model="data.installation_sheet_time" ng-disabled="edit_disable" class="form-control text-center" />
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group">
...
//                            });
    ...
    myApp.directive('jValidate', function () {
        return {
            link: function (scope, element, attr) {
                element.validate({
                    rules: {
                        installation_sheet_date: {
                            required: true,
                            date: true
                        },
                        installation_sheet_time: {
                            required: true,
                            minlength: 5,
                            validTime: true
                        },
                        ...
                        quantity: {
                            ...
                            remote: {
                                url: base_url + '/validate_installation_quantity',
                                type: 'GET',
                                data: {
                                    id: function() {
                                    ...
                                    installation_sheet_id: function() {
                                      return scope.data.installation_sheet_id;
                                    },
                                    item_id: function() {
                    ...
                    messages: {
                        installation_sheet_date: {
                            required: 'Installation Sheet Date is required',
                            date: 'Invalid date format'
                        },
                        installation_sheet_time: {
                            required: 'Installation Sheet Time is required',
    ...
    myApp.controller('mainController', ['$scope', '$http', '$rootScope', 'uiGridConstants', 'uiGridExporterConstants', '$timeout', '$q', '$window', function ($scope, $http, $rootScope, uiGridConstants, uiGridExporterConstants, $timeout, $q, $window) {  
        ...
        $scope.data.installation_sheet_id = <?php echo $installation_sheet_id ? $installation_sheet_id : 0; ?>;
        ...
        $('#installation_sheet_time').mask('00:00');
        ...
        $scope.refreshForm = function(){
            $http({
                ...
                $scope.data = {
                    ...
                    installation_sheet_id: $scope.data.installation_sheet_id,
                    installation_sheet_no: '',
                    installation_sheet_date: new Date(),
                    installation_sheet_time: hh+':'+mm,
                    remarks: '',
                    is_posted: 0,
                    is_approved: 0,
                ...
                $http({
                    ...
                    params: {
                        id: $scope.data.installation_sheet_id
                    }
                }).then(function successCallback(response) {
                    if(response.data.installation_sheet){
                        var installation_sheet_date_time = response.data.installation_sheet.installation_sheet_date_time.split(' ');
                        $scope.data.installation_sheet_id = response.data.installation_sheet.id;
                        $scope.data.installation_sheet_no = response.data.installation_sheet.installation_sheet_no;
                        $scope.data.installation_sheet_date = installation_sheet_date_time[0];
                        $scope.data.installation_sheet_time = installation_sheet_date_time[1];
                        $scope.data.remarks = response.data.installation_sheet.remarks;
                        $scope.data.is_posted = response.data.installation_sheet.is_posted;
                        $scope.data.is_approved = response.data.installation_sheet.is_approved;
                        $scope.edit_disable = response.data.installation_sheet.is_posted == 1 ? true : false;
                    }
        ...
        $scope.editRecord = function (row) {
            $http({
                method: 'GET',
                url: base_url + '/installation_sheet/find_installation_sheet_detail',
                params: {
                    id: row.entity.id
                }
            }).then(function successCallback(response) {
                if(response.data){
                    ...
                    $scope.data = {
                        type: 1,
                        id: response.data.id,
                        inquiry_id: response.data.installation_sheet ? response.data.installation_sheet.inquiry_id : 0,
                        installation_sheet_id: response.data.installation_sheet ? response.data.installation_sheet.id : 0,
                        installation_sheet_no: response.data.installation_sheet ? response.data.installation_sheet.installation_sheet_no : '',
                        installation_sheet_date: installation_sheet_date_time[0],
                        installation_sheet_time: installation_sheet_date_time[1],
                        remarks: response.data.installation_sheet ? response.data.installation_sheet.remarks : '',
                        is_posted: response.data.installation_sheet ? response.data.installation_sheet.is_posted : 0,
            ...
            $http({
                ...
                if(response.data){
                    ...
                    $scope.data = {
                        ...
                        remarks: response.data.installation_sheet ? response.data.installation_sheet.remarks : '',
                        is_posted: response.data.installation_sheet ? response.data.installation_sheet.is_posted : 0,
                        is_approved: response.data.installation_sheet ? response.data.installation_sheet.is_approved : 0,
                        main_category: response.data.item && response.data.item.main_item_category ? {id:response.data.item.main_item_category.id, name:response.data.item.main_item_category.name} : {},
                        sub_category: response.data.item && response.data.item.sub_item_category ? {id:response.data.item.sub_item_category.id, name:response.data.item.sub_item_category.name} : {},
                        code: response.data.item ? {id:response.data.item.id, name:response.data.item.code} : '',
                        item: response.data.item ? {id:response.data.item.id, name:response.data.item.name} : '',
                    ...
                    function(){
                        $http.delete(base_url + '/installation_sheet/'+$scope.data.id, {params: {type: $scope.data.type}}).success(function (response_delete) {
                            swal({
                                title: "Deleted!", 
        ...
        $scope.resetForm = function(){
            $scope.data.installation_sheet_id = '';
            $scope.refreshForm();
            $scope.main_refresh();
        ...
        $scope.postForm = function(){
            swal({
                title: "Are you sure?",
                text: "Installation Sheet No : <strong>"+ $scope.data.installation_sheet_no+"</strong>",
                html: true,
                type: "warning",
                showCancelButton: true,
                confirmButtonColor: "#FF0000",
            ...
            function(){
                ...
                $http({
                    method: 'GET',
                    url: base_url + '/installation_sheet/post_installation_sheet',
                    params: {
                        id: $scope.data.installation_sheet_id
                    }
                }).then(function successCallback(response) {
                    ...
                    swal({
                        title: "Posted!", 
                        text: "Installation Sheet No : "+ $scope.data.installation_sheet_no,
                        html: true,
                        type: "success",
                        confirmButtonColor: "#9ACD32"
                    });
        ...
        $scope.authorizeData = function () {
            ...
            $http({
                method: 'GET',
                url: base_url + '/installation_sheet/get_authorize_data',
                params: {
                    id: $scope.data.inquiry_id
                }
            }).then(function successCallback(response) {
        ...
        $scope.authorizeForm = function(){
            ...
            $http({
                method: 'POST',
                url: base_url + '/installation_sheet/approve_installation_sheet',
                data: {
                    inquiry_id: $scope.data.inquiry_id
                }
            }).then(function successCallback(response) {
        ...
        submitForm = function(){
            ...
            if($scope.data.id == 0){
                $http.post(base_url + '/installation_sheet', $scope.data).success(function (result) {
                    $('#save_button').prop('disabled', false);
                    $.pnotify && $.pnotify({
                        title: 'Installation Sheet Details',
                        text: result.message,
                    ...
                    $scope.data.installation_sheet_id = result.response ? result.data : 0;
                    $scope.refreshForm();
                    $scope.main_refresh();
                });
            } else{
                $http.put(base_url + '/installation_sheet/'+$scope.data.id, $scope.data).success(function (result) {
                    $('#save_button').prop('disabled', false);
                    $.pnotify && $.pnotify({
                        title: 'Installation Sheet Details',
                        text: result.message,
        ...
        $scope.main_refresh = function(){
            ...
            $scope.gridOptions.exporterCsvFilename = $scope.customer_name+' Installation Sheet Details.csv';
            $http({
                method: 'GET',
                url: base_url + '/installation_sheet/installation_sheet_detail_list',
                params: {
                    id: $scope.data.installation_sheet_id
                }
            }).then(function successCallback(response) {
                var data_array = [];
                $.each(response.data.installation_sheet_details, function (index, value) {
                    data_array.push({
                        id: value.id,
                        permission: response.data.permission ? 1 : 0,
                        is_posted: value.installation_sheet ? value.installation_sheet.is_posted : 0,
