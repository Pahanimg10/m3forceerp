...
@section('title')
<title>M3Force | Job Card Details</title>
@endsection

@section('breadcrumb')
<ul class="breadcrumb">
    <li><a href="#">Inquiry</a></li>                    
    <li class="active">Job Card Details</li>
</ul>
@endsection
        ...
        <form autocomplete="off" id="dataForm" name="dataForm" class="form-horizontal" j-validate>
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3 class="panel-title"><strong>Job Card Details</strong></h3>
                    <ul class="panel-controls">
                        ...
                        <li><div><a ng-show="data.job_card_id" target="_blank" class="btn btn-default" ng-href="<%=base_url%>/job_card/print_job_card?id=<%=data.job_card_id%>">Print</a></div></li>
                        ...
                        <li><a href="{{ asset('inquiry/job_card?id='.$inquiry_id.'&type='.$type)}}"><span class="fa fa-times"></span></a></li>
                    </ul>
                </div>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="row">
                                ...
                                <input type="hidden" id="job_card_id" name="job_card_id" ng-model="data.job_card_id" class="form-control" />
                                ...
                                <div class="row">
                                    <h4 class="col-md-12" style="padding-top: 15px;">Job Card Details</h4>
                                </div>
                                ...
                                <div class="row">
                                    <div class="col-md-2">
                                        <div class="form-group">
                                            ...
                                            <input type="text" id="job_card_no" name="job_card_no" ng-model="data.job_card_no" class="form-control text-center" disabled />
                                        </div>
                                    </div>
                                    <div class="col-md-2">
                                        <div class="form-group">
                                            <label class="control-label">Job Card Date</label>
                                            <input type="text" id="job_card_date" name="job_card_date" uib-datepicker-popup="<%=dateFormat%>" ng-model="data.job_card_date" ng-disabled="edit_disable" is-open="jobCardDatePopup.opened" datepicker-options="availableDateOptions" close-text="Close" ng-click="OpenJobCardDate()" class="form-control text-center" />
                                        </div>
                                    </div>
                                    <div class="col-md-2">
                                        <div class="form-group">
                                            <label class="control-label">Job Card Time</label>
                                            <input type="text" id="job_card_time" name="job_card_time" ng-model="data.job_card_time" ng-disabled="edit_disable" class="form-control text-center" />
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="form-group">
...
//                            });
        ...
        return {
            link: function (scope, element, attr) {
                element.validate({
                    ...
                    messages: {
                        job_card_date: {
                            required: 'Job Card Date is required',
                            date: 'Invalid date format'
                        },
                        job_card_time: {
                            required: 'Job Card Time is required',
    ...
    myApp.controller('mainController', ['$scope', '$http', '$rootScope', 'uiGridConstants', 'uiGridExporterConstants', '$timeout', '$q', '$window', function ($scope, $http, $rootScope, uiGridConstants, uiGridExporterConstants, $timeout, $q, $window) {  
        ...
        $scope.data.job_card_id = <?php echo $job_card_id ? $job_card_id : 0; ?>;
        ...
        $scope.jobCardDatePopup = {
            opened: false
        };        
        $scope.OpenJobCardDate = function () {
            $scope.jobCardDatePopup.opened = !$scope.jobCardDatePopup.opened;
        };
        ...
        $scope.refreshForm = function(){
            $http({
                ...
                $scope.data = {
                    ...
                    job_card_id: $scope.data.job_card_id,
                    job_card_no: '',
                    job_card_date: new Date(),
                    job_card_time: hh+':'+mm,
                    remarks: '',
                ...
                $http({
                    method: 'GET',
                    url: base_url + '/job_card/find_job_card',
                    params: {
                        id: $scope.data.job_card_id
                    }
                }).then(function successCallback(response) {
                    if(response.data){
                        var job_card_date_time = response.data.job_card_date_time.split(' ');
                        $scope.data.job_card_id = response.data.id;
                        $scope.data.job_card_no = response.data.job_card_no;
                        $scope.data.job_card_date = job_card_date_time[0];
                        $scope.data.job_card_time = job_card_date_time[1];
        ...
        $scope.editRecord = function (row) {
            $http({
                method: 'GET',
                url: base_url + '/job_card/find_job_card_detail',
                params: {
                    id: row.entity.id
                }
            }).then(function successCallback(response) {
                if(response.data){
                    var job_card_date_time = response.data.job_card.job_card_date_time.split(' ');
                    var margin = (Number(response.data.margin)+100)/100;
                    $scope.data = {
                        type: 1,
                        id: response.data.id,
                        inquiry_id: response.data.job_card ? response.data.job_card.inquiry_id : 0,
                        job_card_id: response.data.job_card ? response.data.job_card.id : 0,
                        job_card_no: response.data.job_card ? response.data.job_card.job_card_no : '',
                        job_card_date: job_card_date_time[0],
                        job_card_time: job_card_date_time[1],
                        remarks: response.data.job_card ? response.data.job_card.remarks : '',
        ...
        $scope.deleteRecord = function (row) {
            $http({
                ...
                if(response.data){
                    ...
                    function(){
                        $http.delete(base_url + '/job_card/'+$scope.data.id, {params: {type: $scope.data.type}}).success(function (response_delete) {
                            swal({
                                title: "Deleted!", 
        ...
        submitForm = function(){
            ...
            if($scope.data.id == 0){
                $http.post(base_url + '/job_card', $scope.data).success(function (result) {
                    ...
                    $scope.data.job_card_id = result.response ? result.data : 0;
                    $scope.refreshForm();
                    $scope.main_refresh();
                });
            } else{
                $http.put(base_url + '/job_card/'+$scope.data.id, $scope.data).success(function (result) {
                    $('#save_button').prop('disabled', false);
                    $.pnotify && $.pnotify({
                        title: 'Job Card Details',
                        text: result.message,
        ...
        $scope.main_refresh = function(){
            ...
            $scope.gridOptions.exporterCsvFilename = $scope.customer_name+' Job Card Details.csv';
            $http({
                method: 'GET',
                url: base_url + '/job_card/job_card_detail_list',
                params: {
                    id: $scope.data.job_card_id
                }
            }).then(function successCallback(response) {
                var data_array = [];
                $.each(response.data.job_card_details, function (index, value) {
                    var margin = (Number(value.margin)+100)/100;
                    data_array.push({
                        id: value.id,
                        permission: response.data.permission ? 1 : 0,
                        is_used: response.data.job_card ? response.data.job_card.is_used : 0,
                ...
                $scope.edit_disable = response.data.job_card ? response.data.job_card.is_used == 1 || $scope.edit_disable : $scope.edit_disable; 
