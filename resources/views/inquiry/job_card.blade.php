    ...
    myApp.controller('mainController', ['$scope', '$http', '$rootScope', 'uiGridConstants', 'uiGridExporterConstants', '$timeout', '$q', '$window', function ($scope, $http, $rootScope, uiGridConstants, uiGridExporterConstants, $timeout, $q, $window) {  
        ...
        $scope.gridOptions = {
            columnDefs: [
                ...
                {
                    field: 'job_card_date_time', 
                    displayName: 'Job Card Date & Time', 
                    cellClass: 'grid-align',
                    width: '15%', 
                    enableCellEdit: false
                },
                ...
                {
                    field: 'job_card_value', 
                    displayName: 'Job Card Value', 
        ...
        $scope.getAggregationJobCardTotalValue = function(){
            ...
            for(var i=0; i<$scope.gridOptions.data.length; i++){
                total_value += Number($scope.gridOptions.data[i].job_card_value);
            }
            return total_value;
        };
        ...
        $scope.main_refresh = function(){
            ...
            $http({
                method: 'GET',
                url: base_url + '/job_card/job_card_list',
                params: {
                ...
                $.each(response.data.job_cards, function (index, value) {
                    data_array.push({
                        ...
                        job_card_date_time: value.job_card_date_time,
                        remarks: value.remarks,
                        job_card_value: value.job_card_value != 0 ? parseFloat(Math.round(value.job_card_value * 100) / 100).toFixed(2) : '',
                        confirmed: value.is_used == 1 ? 'Yes' : 'No',
                        log_user: value.user ? value.user.first_name : ''
                    });
                });    
                $scope.gridOptions.data = data_array;
                $scope.gridOptions.exporterCsvFilename = response.data.inquiry ? response.data.inquiry.inquiry_no + ' Job Cards.csv' : 'Job Cards.csv';
        ...
        $scope.add_new = function () {
            $window.location.href = base_url + '/job_card/add_new?type='+type+'&view=0&inquiry_id='+$scope.inquiry_id;
        };

        $scope.editRecord = function (row) {
            $window.location.href = base_url + '/job_card/add_new?type='+type+'&view=0&id='+row.entity.id+'&inquiry_id='+$scope.inquiry_id;
        };
        ...
        $scope.deleteRecord = function (row) {
            $http({
                ...
                swal({
                    title: "Are you sure?",
                    text: "You will not be able to recover <strong>" + response.data.job_card_no + "</strong> job card !",
                    html: true,
                    type: "warning",
                ...
                function(){
                    $http.delete(base_url + '/job_card/'+row.entity.id, {params: {type: 0}}).success(function (response_delete) {
                        swal({
                            title: "Deleted!", 
                            text: response.data.job_card_no + " job card has been deleted.", 
                            type: "success",
