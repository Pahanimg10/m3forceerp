...
@section('title')
<title>M3Force | Print Inventory Return</title>
@endsection
...
<div class="panel panel-default">
    <div class="panel-heading">
        <h4 style="margin-top: 5px; margin-bottom: 5px; float: right; font-family: Verdana, Geneva, sans-serif; font-size: 14px;">Inventory Return</h4>
    </div>
    <div class="panel-body">
        <table id="header_table">
            <tr>
                <td>Inventory Issue No</td>
                <td>{{ $inventory_return->InventoryIssue->inventory_issue_no }}</td>
                <td>Issued To</td>
                <td>{{ $inventory_return->InventoryIssue->issued_to }}</td>
            </tr>
            <tr>
                <td>Inventory Return No</td>
                <td>{{ $inventory_return->inventory_return_no }}</td>
                <td>Date & Time</td>
                <td>{{ $inventory_return->inventory_return_date_time }}</td>
            </tr>
            <tr>
                <td>Remarks</td>
                <td colspan="3">{{ $inventory_return->remarks }}</td>
            </tr>
        </table>
    </div>
</div>
...
<div class="panel panel-default" style="margin-top: 10px;">
    <div class="panel-body">
        <table id="detail_table">
            ...
            {{--*/ $s = 1 /*--}}
            {{--*/ $total_value = 0 /*--}}
            @if ($inventory_return->inventoryReturnDetails)
                @foreach ($inventory_return->inventoryReturnDetails as $index => $value)
