...
@section('title')
<title>M3Force | Inventory Issue Details</title>
@endsection

@section('breadcrumb')
<ul class="breadcrumb">
    <li><a href="#">Inventory</a></li>                    
    <li class="active">Inventory Issue Details</li>
</ul>
@endsection
        ...
        <form autocomplete="off" id="dataForm" name="dataForm" class="form-horizontal" j-validate>
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3 class="panel-title"><strong>Inventory Issue Details</strong></h3>
                    <ul class="panel-controls">
                        ...
                        <li><button type="button" ng-show="data.inventory_issue_id" ng-disabled="edit_disable || data.is_posted == 1" ng-click="postForm()" class="btn btn-danger" id="post_button">Post</button></li>
                        ...
                        <li><div><a ng-show="data.inventory_issue_id && data.is_posted == 1" target="_blank" class="btn btn-default" ng-href="<%=base_url%>/inventory_issue/print_inventory_issue?id=<%=data.inventory_issue_id%>">Print</a></div></li>
                        ...
                        <li><a href="{{ asset('inventory_issue')}}"><span class="fa fa-times"></span></a></li>
                    </ul>
                </div>
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <div class="row">
                                ...
                                <input type="hidden" id="inventory_issue_id" name="inventory_issue_id" ng-model="data.inventory_issue_id" class="form-control" />
                                ...
                                <div class="row">
                                    <h4 class="col-md-12" style="padding-top: 15px;">Inventory Issue Details</h4>
                                </div>
                                ...
                                <div class="row">
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            ...
                                            <input type="text" id="inventory_issue_no" name="inventory_issue_no" ng-model="data.inventory_issue_no" class="form-control text-center" disabled />
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            <label class="control-label">Inventory Issue Date</label>
                                            <input type="text" id="inventory_issue_date" name="inventory_issue_date" uib-datepicker-popup="<%=dateFormat%>" ng-model="data.inventory_issue_date" ng-disabled="edit_disable" is-open="inventoryIssueDatePopup.opened" datepicker-options="availableDateOptions" close-text="Close" ng-click="OpenInventoryIssueDate()" class="form-control text-center" />
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="form-group">
                                            <label class="control-label">Inventory Issue Time</label>
                                            <input type="text" id="inventory_issue_time" name="inventory_issue_time" ng-model="data.inventory_issue_time" ng-disabled="edit_disable" class="form-control text-center" />
                                        </div>
                                    </div>
                                    <div class="col-md-3">
                                        <div class="form-group">
...
<script type="text/javascript">
    ...
    myApp.controller('menuController', function ($scope) {
        ...
        angular.element(document.querySelector('#sub_menu_inventory_issue')).addClass('active');
    });  
...
//                            });
    ...
    myApp.directive('jValidate', function () {
        return {
            link: function (scope, element, attr) {
                element.validate({
                    rules: {
                        inventory_issue_date: {
                            required: true,
                            date: true
                        },
                        inventory_issue_time: {
                            required: true,
                            minlength: 5,
                            validTime: true
                        },
                        ...
                        inventory_code: {
                            remote: {
                                url: base_url + '/inventory_issue/validate_inventory_code',
                                type: 'GET',
                                data: {
                    ...
                    messages: {
                        inventory_issue_date: {
                            required: 'Inventory Issue Date is required',
                            date: 'Invalid date format'
                        },
                        inventory_issue_time: {
                            required: 'Inventory Issue Time is required',
                        ...
                        inventory_code: {
                            remote: 'Invalid Inventory Code or Inventory not available'
                        }
                    },
    ...
    myApp.controller('mainController', ['$scope', '$http', '$rootScope', 'uiGridConstants', 'uiGridExporterConstants', '$timeout', '$q', '$window', function ($scope, $http, $rootScope, uiGridConstants, uiGridExporterConstants, $timeout, $q, $window) {  
        ...
        $scope.data.inventory_issue_id = <?php echo $inventory_issue_id ? $inventory_issue_id : 0; ?>;
        ...
        $scope.inventoryIssueDatePopup = {
            opened: false
        };        
        $scope.OpenInventoryIssueDate = function () {
            $scope.inventoryIssueDatePopup.opened = !$scope.inventoryIssueDatePopup.opened;
        };
        
        $('#inventory_issue_time').mask('00:00');
        ...
        $scope.refreshForm = function(){
            $http({
                ...
                $scope.data = {
                    id: 0,
                    inventory_issue_id: $scope.data.inventory_issue_id,
                    inventory_code_value: '',
                    inventory_name_value: '',
                    inventory_issue_no: '',
                    inventory_issue_date: new Date(),
                    inventory_issue_time: hh+':'+mm,
                    issued_to: '',
                    remarks: '',
                    is_posted: 0,
                ...
                $http({
                    method: 'GET',
                    url: base_url + '/inventory_issue/find_inventory_issue',
                    params: {
                        id: $scope.data.inventory_issue_id
                    }
                }).then(function successCallback(response) {
                    if(response.data){
                        var inventory_issue_date_time = response.data.inventory_issue_date_time.split(' ');
                        $scope.data.inventory_issue_id = response.data.id;
                        $scope.data.inventory_issue_no = response.data.inventory_issue_no;
                        $scope.data.inventory_issue_date = inventory_issue_date_time[0];
                        $scope.data.inventory_issue_time = inventory_issue_date_time[1];
        ...
        $scope.editRecord = function (row) {
            $http({
                method: 'GET',
                url: base_url + '/inventory_issue/find_inventory_issue_detail',
                params: {
                    id: row.entity.id
                }
            }).then(function successCallback(response) {
                if(response.data){ 
                    var inventory_issue_date_time = response.data.inventory_issue.inventory_issue_date_time.split(' ');
                    $scope.data = {
                        id: response.data.id,
                        inventory_issue_id: response.data.inventory_issue ? response.data.inventory_issue.id : 0,
                        inventory_code_value: response.data.inventory_register ? response.data.inventory_register.code : '',
                        inventory_issue_no: response.data.inventory_issue ? response.data.inventory_issue.inventory_issue_no : '',
                        inventory_issue_date: inventory_issue_date_time[0],
                        inventory_issue_time: inventory_issue_date_time[1],
                        issued_to: response.data.inventory_issue ? response.data.inventory_issue.issued_to : '',
                        remarks: response.data.inventory_issue ? response.data.inventory_issue.remarks : '',
                        is_posted: response.data.inventory_issue ? response.data.inventory_issue.is_posted : 0,
        ...
        $scope.deleteRecord = function (row) {
            $http({
                ...
                if(response.data){ 
                    ...
                    function(){
                        $http.delete(base_url + '/inventory_issue/'+$scope.data.id, {params: {type: 1}}).success(function (response_delete) {
                            swal({
                                title: "Deleted!", 
        ...
        $scope.postForm = function(){
            swal({
                title: "Are you sure?",
                text: "Inventory Issue No : <strong>"+ $scope.data.inventory_issue_no+"</strong>",
                html: true,
                type: "warning",
                showCancelButton: true,
                confirmButtonColor: "#FF0000",
            ...
            function(){
                ...
                $http({
                    method: 'GET',
                    url: base_url + '/inventory_issue/post_inventory_issue',
                    params: {
                        id: $scope.data.inventory_issue_id
                    }
                    ...
                    if(result.data.response){
                        swal({
                            title: "Posted!", 
                            text: "Inventory Issue No : "+ $scope.data.inventory_issue_no,
                            html: true,
                            type: "success",
                            confirmButtonColor: "#9ACD32"
                        });
        ...
        submitForm = function(){
            ...
            if($scope.data.id == 0){
                $http.post(base_url + '/inventory_issue', $scope.data).success(function (result) {
                    $('#save_button').prop('disabled', false);
                    $.pnotify && $.pnotify({
                        title: 'Inventory Issue Details',
                        text: result.message,
                    ...
                    $scope.data.inventory_issue_id = result.response ? result.data : 0;
                    $scope.refreshForm();
                    $scope.main_refresh();
                });
            } else{
                $http.put(base_url + '/inventory_issue/'+$scope.data.id, $scope.data).success(function (result) {
                    $('#save_button').prop('disabled', false);
                    $.pnotify && $.pnotify({
                        title: 'Inventory Issue Details',
                        text: result.message,
        ...
        $scope.main_refresh = function(){
            ...
            $scope.gridOptions.exporterCsvFilename = $scope.data.inventory_issue_no+' Inventory Issue Details.csv';
            $http({
                method: 'GET',
                url: base_url + '/inventory_issue/inventory_issue_detail_list',
                params: {
                    id: $scope.data.inventory_issue_id
                }
            }).then(function successCallback(response) {
                ...
                $.each(response.data, function (index, value) {
                    data_array.push({
                        id: value.id,
                        is_posted: value.inventory_issue ? value.inventory_issue.is_posted : 0,
                        ...
                        inventory_remarks: value.remarks
                    });
                });    
