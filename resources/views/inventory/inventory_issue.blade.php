...
@section('title')
<title>M3Force | Inventory Issue</title>
@endsection

@section('breadcrumb')
<ul class="breadcrumb">
    <li><a href="#">Inventory</a></li>                    
    <li class="active">Inventory Issue</li>
</ul>
@endsection
        ...
        <form autocomplete="off" id="dataForm" name="dataForm" class="form-horizontal" j-validate>
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3 class="panel-title"><strong>Inventory Issue</strong></h3>
    ...
    myApp.controller('mainController', ['$scope', '$http', '$rootScope', 'uiGridConstants', 'uiGridExporterConstants', '$timeout', '$q', '$window', function ($scope, $http, $rootScope, uiGridConstants, uiGridExporterConstants, $timeout, $q, $window) {  
        ...
        $scope.gridOptions = {
            columnDefs: [
                ...
                {
                    field: 'inventory_issue_no', 
                    displayName: 'Inventory Issue No', 
                    cellClass: 'grid-align',
                    width: '15%', 
                    enableCellEdit: false
                },
                {
                    field: 'inventory_issue_date_time', 
                    displayName: 'Inventory Issue Date & Time', 
                    cellClass: 'grid-align',
                    width: '20%', 
                    enableCellEdit: false
                },
                ...
                {
                    field: 'inventory_issue_value', 
                    displayName: 'Inventory Issue Value', 
                    cellClass: 'grid-align-right',
                    width: '15%', 
                    footerCellTemplate: '<div class="ui-grid-cell-contents text-right" ><%= grid.appScope.getAggregationTotalInventoryIssue() | number:2 %></div>',
                    enableCellEdit: false
                },
                {
                    field: 'remarks', 
        ...
        $scope.getAggregationTotalInventoryIssue = function(){
            var total_value = 0;
            for(var i=0; i<$scope.gridOptions.data.length; i++){
                total_value += Number($scope.gridOptions.data[i].inventory_issue_value);
            }
            return total_value;
        };
        ...
        $scope.main_refresh = function(){
            ...
            $scope.gridOptions.exporterCsvFilename = 'Inventory Issues From ' + $scope.data.from + ' To ' + $scope.data.to + '.csv';
            $http({
                method: 'GET',
                url: base_url + '/inventory_issue/inventory_issue_list',
                params: {
                ...
                $.each(response.data, function (index, value) {
                    data_array.push({
                        ...
                        inventory_issue_no: value.inventory_issue_no,
                        inventory_issue_date_time: value.inventory_issue_date_time,
                        issued_to: value.issued_to,
                        inventory_issue_value: value.inventory_issue_value != 0 ? parseFloat(Math.round(value.inventory_issue_value * 100) / 100).toFixed(2) : '',
        ...
        $scope.add_new = function () {
            $window.location.href = base_url + '/inventory_issue/add_new';
        };

        $scope.editRecord = function (row) {
            $window.location.href = base_url + '/inventory_issue/add_new?id='+row.entity.id;
        };
        ...
        $scope.deleteRecord = function (row) {
            $http({
                ...
                swal({
                    title: "Are you sure?",
                    text: "You will not be able to recover <strong>" + response.data.inventory_issue_no + "</strong> inventory issue !",
                    html: true,
                    type: "warning",
                ...
                function(){
                    $http.delete(base_url + '/inventory_issue/'+row.entity.id, {params: {type: 0}}).success(function (response_delete) {
                        swal({
                            title: "Deleted!", 
                            text: response.data.inventory_issue_no + " inventory issue has been deleted.", 
                            type: "success",
