...
@section('title')
<title>M3Force | Tech Response Customer Details</title>
@endsection

@section('breadcrumb')
<ul class="breadcrumb">
    <li><a href="#">Collection</a></li>                    
    <li class="active">Tech Response Customer Details</li>
</ul>
@endsection
        ...
        <form autocomplete="off" id="dataForm" name="dataForm" class="form-horizontal" j-validate>
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3 class="panel-title"><strong>Tech Response Customer Details</strong></h3>
                    <ul class="panel-controls">
                        ...
                        <li><a href="{{ asset('tech_response_customer')}}"><span class="fa fa-times"></span></a></li>
                    </ul>
                </div>
                <div class="panel-body">       
                    <div class="row">
                        <input type="hidden" id="tech_response_customer_id" name="tech_response_customer_id" ng-model="data.tech_response_customer_id" class="form-control" />
                        <div class="col-md-6">
...
<script type="text/javascript">
    ...
    myApp.controller('menuController', function ($scope) {
        ...
        angular.element(document.querySelector('#sub_menu_tech_response_customer')).addClass('active');
    });  
    ...
    myApp.controller('mainController', ['$scope', '$http', '$rootScope', 'uiGridConstants', 'uiGridExporterConstants', '$timeout', '$q', '$window', function ($scope, $http, $rootScope, uiGridConstants, uiGridExporterConstants, $timeout, $q, $window) {  
        $scope.tech_response_customer_id = <?php echo $tech_response_customer_id ? $tech_response_customer_id : 0; ?>;
        ...
        $scope.invoiceGridOptions = {
            columnDefs: [
                ...
                },
                {
                    ...
                    width: '40%',  
                    enableCellEdit: false,
                    cellTemplate: '<div class="ui-grid-cell-contents"><a ng-href="'+base_url+'/tech_response_customer/print_tech_response_customer_invoice?id=<%= row.entity.id %>" target="_blank"><%= row.entity.invoice_no %></a></div>'
                },
                {
                    field: 'invoice_value', 
        ...
        $scope.paymentGridOptions = {
            columnDefs: [
                ...
                },
                {
                    ...
                    width: '30%',  
                    enableCellEdit: false,
                    cellTemplate: '<div class="ui-grid-cell-contents"><a ng-href="'+base_url+'/tech_response_customer/print_tech_response_customer_payment?id=<%= row.entity.id %>" target="_blank"><%= row.entity.receipt_no %></a></div>'
                },
                {
                    field: 'amount', 
                    displayName: 'Amount', 
        ...
        $scope.main_refresh = function(){
            ...
            $http({
                method: 'GET',
                url: base_url + '/tech_response_customer/get_tech_response_customer_detail',
                params: {
                    id: $scope.tech_response_customer_id
                }
            }).then(function successCallback(response) {
                var invoice_data_array = [];
                var payment_data_array = [];
                $.each(response.data.tech_response_customer_invoices, function (index, value) {
                    if(value.tech_response_quotation){
                        invoice_data_array.push({
                            id: value.id,
                            invoice_date: value.invoice_date,
                            invoice_no: value.invoice_no,
                            invoice_value: parseFloat(Math.round(value.tech_response_quotation.tech_response_quotation_value * 100) / 100).toFixed(2)
                        });
                    }
                });   
                $.each(response.data.tech_response_customer_payments, function (index, value) { 
                ...
                $scope.invoiceGridOptions.exporterCsvFilename = response.data.tech_response_customer && response.data.tech_response_customer.contact ? response.data.tech_response_customer.contact.name+' Invoice Details.csv' : 'Invoice Details.csv';
                $scope.invoiceGridOptions.data = invoice_data_array;
                $scope.paymentGridOptions.exporterCsvFilename = response.data.tech_response_customer && response.data.tech_response_customer.contact ? response.data.tech_response_customer.contact.name+' Payment Details.csv' : 'Payment Details.csv';
        ...
        $scope.add_new_payment = function () {
            $window.location.href = base_url + '/tech_response_customer/add_new_payment?tech_response_customer_id='+$scope.tech_response_customer_id;
        };

        $scope.editRecord = function (row) {
            $window.location.href = base_url + '/tech_response_customer/add_new_payment?id='+row.entity.id+'&tech_response_customer_id='+$scope.tech_response_customer_id;
        };
        ...
        $scope.deleteRecord = function (row) {
            ...
            function(){
                $http.delete(base_url + '/tech_response_customer/'+row.entity.id, row.entity).success(function (response_delete) {
                    swal({
                        title: "Deleted!", 
