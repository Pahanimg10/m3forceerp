...
@section('title')
<title>M3Force | Tech Response Customer Payment</title>
@endsection

@section('breadcrumb')
<ul class="breadcrumb">
    <li><a href="#">Collection</a></li>                    
    <li class="active">Tech Response Customer Payment</li>
</ul>
@endsection
            ...
            <div class="panel panel-default">
                <div class="panel-heading">
                    ...
                    <ul class="panel-controls">
                        ...
                        <li><div><a ng-show="data.id" target="_blank" class="btn btn-default" ng-href="<%=base_url%>/tech_response_customer/print_job_done_payment?id=<%=data.id%>">Print</a></div></li>
                        <li><a href="{{ asset('tech_response_customer/tech_response_customer_detail?id='.$tech_response_customer_id)}}"><span class="fa fa-times"></span></a></li>
                    </ul>
                </div>
...
<script type="text/javascript">
    ...
    myApp.controller('mainController', ['$scope', '$http', '$rootScope', 'uiGridConstants', 'uiGridExporterConstants', '$timeout', '$q', '$window', function ($scope, $http, $rootScope, uiGridConstants, uiGridExporterConstants, $timeout, $q, $window) {  
        ...
        $http({
            method: 'GET',
            url: base_url + '/tech_response_customer/get_data'
            ...
            $scope.data = {
                id: <?php echo $tech_response_customer_payment_id ? $tech_response_customer_payment_id : 0; ?>,
                tech_response_customer_id: <?php echo $tech_response_customer_id ? $tech_response_customer_id : 0; ?>,
            ...
            $http({
                ...
                params: {
                    id: $scope.data.tech_response_customer_id
                }
            }).then(function successCallback(response) {
                if(response.data.tech_response_customer){
                    $scope.customer_name = response.data.tech_response_customer.contact ? response.data.tech_response_customer.contact.name : '';
                    $scope.customer_address = response.data.tech_response_customer.contact ? response.data.tech_response_customer.contact.address : '';
                    $scope.customer_contact_no = response.data.tech_response_customer.contact ? response.data.tech_response_customer.contact.contact_no : '';
                }
        ...
        submitForm = function(){
            ...
            if($scope.data.id == 0){
                $http.post(base_url + '/tech_response_customer', $scope.data).success(function (result) {
                    $('#save_button').prop('disabled', false);
                    $.pnotify && $.pnotify({
                        title: 'Tech Response Customer Payments',
                        text: result.message,
                ...
                $http.put(base_url + '/tech_response_customer/'+$scope.data.id, $scope.data).success(function (result) {
                    $('#save_button').prop('disabled', false);
                    $.pnotify && $.pnotify({
                        title: 'Tech Response Customer Payments',
                        text: result.message,
        ...
        $scope.main_refresh = function(){
            ...
            $http({
                method: 'GET',
                url: base_url + '/tech_response_customer/find_tech_response_customer_payment',
                params: {
                    id: $scope.data.id
                }
            }).then(function successCallback(response) {
                if(response.data){
                    ...
                    $scope.data = {
                        id: response.data.id,
                        tech_response_customer_id: response.data.tech_response_customer_id,
