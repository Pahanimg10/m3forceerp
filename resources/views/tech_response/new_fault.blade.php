    ...
    myApp.controller('mainController', ['$scope', '$http', '$rootScope', 'uiGridConstants', 'uiGridExporterConstants', '$timeout', '$q', '$window', function ($scope, $http, $rootScope, uiGridConstants, uiGridExporterConstants, $timeout, $q, $window) {  
        ...
        $scope.gridOptions = {
            columnDefs: [
                ...
                },
                {
                    ...
                    width: '20%', 
                    enableCellEdit: false,
                    cellTemplate: '<div class="ui-grid-cell-contents"><a ng-href="'+base_url+'/tech_response/add_new_tech_response?contact_id=<%= row.entity.id %>" target="_self"><%= row.entity.name %></a></div>'
                },
                {
                    field: 'address', 
                    displayName: 'Address', 
        ...
        $scope.main_refresh = function(){
            ...
            $http({
                ...
                params: {
                    contact_type: $scope.data.contact_type ? $scope.data.contact_type.id : 1
                }
            }).then(function successCallback(response) {
                var data_array = [];
        ...
        $scope.add_new = function () {
            $window.location.href = base_url + '/tech_response/add_new_contact';
        };
    }]);
</script>
@endsection