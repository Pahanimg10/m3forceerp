        ...
        <form autocomplete="off" id="dataForm" name="dataForm" class="form-horizontal" j-validate>
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3 class="panel-title"><strong>Job Card</strong></h3>
    ...
    myApp.controller('mainController', ['$scope', '$http', '$rootScope', 'uiGridConstants', 'uiGridExporterConstants', '$timeout', '$q', '$window', function ($scope, $http, $rootScope, uiGridConstants, uiGridExporterConstants, $timeout, $q, $window) {  
        ...
        $scope.gridOptions = {
            columnDefs: [
                ...
                {
                    field: 'tech_response_job_card_no', 
                    displayName: 'Tech Response Job Card No', 
                    cellClass: 'grid-align',
                    width: '20%', 
                    enableCellEdit: false
                },
                {
                    field: 'tech_response_job_card_date_time', 
                    displayName: 'Tech Response Job Card Date & Time', 
                    cellClass: 'grid-align',
                    width: '20%', 
                    enableCellEdit: false
                },
                ...
                {
                    field: 'tech_response_job_card_value', 
                    displayName: 'Tech Response Job Card Value', 
        ...
        $scope.getAggregationJobCardTotalValue = function(){
            ...
            for(var i=0; i<$scope.gridOptions.data.length; i++){
                total_value += Number($scope.gridOptions.data[i].tech_response_job_card_value);
            }
            return total_value;
        };
        ...
        $scope.main_refresh = function(){
            ...
            $http({
                method: 'GET',
                url: base_url + '/tech_response_job_card/tech_response_job_card_list',
                params: {
                ...
                $.each(response.data.tech_response_job_cards, function (index, value) {
                    data_array.push({
                        ...
                        tech_response_job_card_no: value.tech_response_job_card_no,
                        tech_response_job_card_date_time: value.tech_response_job_card_date_time,
                        remarks: value.remarks,
                        tech_response_job_card_value: value.tech_response_job_card_value != 0 ? parseFloat(Math.round(value.tech_response_job_card_value * 100) / 100).toFixed(2) : '',
                        log_user: value.user ? value.user.first_name : ''
                    });
                });    
                $scope.gridOptions.data = data_array;
                $scope.gridOptions.exporterCsvFilename = response.data.tech_response ? response.data.tech_response.tech_response_no + ' Tech Response Job Cards.csv' : 'Tech Response Job Cards.csv';
        ...
        $scope.add_new = function () {
            $window.location.href = base_url + '/tech_response_job_card/add_new?view=0&tech_response_id='+$scope.tech_response_id;
        };

        $scope.editRecord = function (row) {
            $window.location.href = base_url + '/tech_response_job_card/add_new?view=0&id='+row.entity.id+'&tech_response_id='+$scope.tech_response_id;
        };
        ...
        $scope.deleteRecord = function (row) {
            $http({
                ...
                swal({
                    title: "Are you sure?",
                    text: "You will not be able to recover <strong>" + response.data.tech_response_job_card_no + "</strong> job card !",
                    html: true,
                    type: "warning",
                ...
                function(){
                    $http.delete(base_url + '/tech_response_job_card/'+row.entity.id, {params: {type: 0}}).success(function (response_delete) {
                        swal({
                            title: "Deleted!", 
                            text: response.data.tech_response_job_card_no + " job card has been deleted.", 
                            type: "success",
