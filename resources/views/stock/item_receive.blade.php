...
@section('title')
<title>M3Force | Item Return</title>
@endsection

@section('breadcrumb')
<ul class="breadcrumb">
    <li><a href="#">Stock</a></li>
    <li class="active">Item Return</li>
</ul>
@endsection
        ...
        <form autocomplete="off" id="dataForm" name="dataForm" class="form-horizontal" j-validate>
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3 class="panel-title"><strong>Item Return</strong></h3>
    ...
    myApp.controller('mainController', ['$scope', '$http', '$rootScope', 'uiGridConstants', 'uiGridExporterConstants', '$timeout', '$q', '$window', function($scope, $http, $rootScope, uiGridConstants, uiGridExporterConstants, $timeout, $q, $window) {
        ...
        $scope.gridOptions = {
            columnDefs: [{
                ...
                {
                    field: 'item_receive_no',
                    displayName: 'Item Return No',
                    cellClass: 'grid-align',
                    width: '15%',
                    enableCellEdit: false
                },
                {
                    field: 'item_receive_date_time',
                    displayName: 'Item Return Date & Time',
                    cellClass: 'grid-align',
                    width: '20%',
                    enableCellEdit: false
                },
                ...
                {
                    field: 'item_receive_value',
                    displayName: 'Item Return Value',
        ...
        $scope.getAggregationItemIssueTotalValue = function() {
            ...
            for (var i = 0; i < $scope.gridOptions.data.length; i++) {
                total_value += Number($scope.gridOptions.data[i].item_receive_value);
            }
            return total_value;
        };
        ...
        $scope.main_refresh = function() {
            ...
            $scope.gridOptions.exporterCsvFilename = 'Item Returns From ' + $scope.data.from + ' To ' + $scope.data.to + '.csv';
            $http({
                method: 'GET',
                url: base_url + '/item_receive/item_receive_list',
                params: {
                ...
                var data_array = [];
                $.each(response.data.item_receives, function(index, value) {
                    var document_no = '';
                    var customer = '';
                    document_no = value.item_issue && value.item_issue.item_issue_type_id == 1 && value.item_issue.job ? value.item_issue.job.job_no : document_no;
                    document_no = value.item_issue && value.item_issue.item_issue_type_id == 2 && value.item_issue.tech_response ? value.item_issue.tech_response.tech_response_no : document_no;
                    customer = value.item_issue && value.item_issue.item_issue_type_id == 1 && value.item_issue.job && value.item_issue.job.inquiry && value.item_issue.job.inquiry.contact ? value.item_issue.job.inquiry.contact.name : customer;
                    customer = value.item_issue && value.item_issue.item_issue_type_id == 2 && value.item_issue.tech_response && value.item_issue.tech_response.contact ? value.item_issue.tech_response.contact.name : customer;
                    data_array.push({
                        id: value.id,
                        permission: response.data.permission ? 1 : 0,
                        is_posted: value.is_posted,
                        item_receive_no: value.item_receive_no,
                        item_receive_date_time: value.item_receive_date_time,
                        item_issue_no: value.item_issue ? value.item_issue.item_issue_no : '',
                        document_no: document_no,
                        issued_to: customer != '' ? value.item_issue ? customer + ' : ' + value.item_issue.issued_to : customer : '',
                        remarks: value.remarks,
                        item_receive_value: parseFloat(Math.round(value.item_receive_value * 100) / 100).toFixed(2)
                    });
                });
        ...
        $scope.add_new = function() {
            $window.location.href = base_url + '/item_receive/add_new';
        };

        $scope.editRecord = function(row) {
            $window.open(base_url + '/item_receive/add_new?id=' + row.entity.id, '_blank');
        };
        ...
        $scope.deleteRecord = function(row) {
            $http({
                ...
                swal({
                        title: "Are you sure?",
                        text: "You will not be able to recover <strong>" + response.data.item_receive_no + "</strong> Item Return !",
                        html: true,
                        type: "warning",
                    ...
                    function() {
                        $http.delete(base_url + '/item_receive/' + row.entity.id, {
                            params: {
                                type: 0
                            }
                            ...
                            swal({
                                title: "Deleted!",
                                text: response.data.item_receive_no + " Item Return has been deleted.",
                                type: "success",
