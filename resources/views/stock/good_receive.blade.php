...
@section('title')
<title>M3Force | Good Receive</title>
@endsection

@section('breadcrumb')
<ul class="breadcrumb">
    <li><a href="#">Stock</a></li>
    <li class="active">Good Receive</li>
</ul>
@endsection
        ...
        <form autocomplete="off" id="dataForm" name="dataForm" class="form-horizontal" j-validate>
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3 class="panel-title"><strong>Good Receive</strong></h3>
    ...
    myApp.controller('mainController', ['$scope', '$http', '$rootScope', 'uiGridConstants', 'uiGridExporterConstants', '$timeout', '$q', '$window', function($scope, $http, $rootScope, uiGridConstants, uiGridExporterConstants, $timeout, $q, $window) {
        ...
        $scope.getAggregationGoodReceiveTotalValue = function() {
            ...
            for (var i = 0; i < $scope.gridOptions.data.length; i++) {
                total_value += Number($scope.gridOptions.data[i].good_receive_value);
            }
            return total_value;
        };
        ...
        $scope.main_refresh = function() {
            ...
            $scope.gridOptions.exporterCsvFilename = 'Good Receives From ' + $scope.data.from + ' To ' + $scope.data.to + '.csv';
            $http({
                method: 'GET',
                url: base_url + '/good_receive/good_receive_list',
                params: {
                ...
                var data_array = [];
                $.each(response.data.good_receives, function(index, value) {
                    data_array.push({
                        id: value.id,
                        ...
                        supplier: value.purchase_order && value.purchase_order.contact ? value.purchase_order.contact.name : '',
                        purchase_order_no: value.purchase_order ? value.purchase_order.purchase_order_no : '',
                        invoice_no: value.invoice_no,
                        remarks: value.remarks,
                        good_receive_value: value.good_receive_value != 0 ? parseFloat(Math.round(value.good_receive_value * 100) / 100).toFixed(2) : ''
                    });
                });
        ...
        $scope.add_new = function() {
            $window.location.href = base_url + '/good_receive/add_new';
        };

        $scope.editRecord = function(row) {
            $window.open(base_url + '/good_receive/add_new?id=' + row.entity.id, '_blank');
        };
        ...
        $scope.deleteRecord = function(row) {
            $http({
                ...
                swal({
                        title: "Are you sure?",
                        text: "You will not be able to recover <strong>" + response.data.good_receive_no + "</strong> purchase order !",
                        html: true,
                        type: "warning",
                    ...
                    function() {
                        $http.delete(base_url + '/good_receive/' + row.entity.id, {
                            params: {
                                type: 0
                            }
                            ...
                            swal({
                                title: "Deleted!",
                                text: response.data.good_receive_no + " purchase order has been deleted.",
                                type: "success",
