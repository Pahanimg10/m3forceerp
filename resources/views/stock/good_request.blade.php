...
@section('title')
<title>M3Force | Good Request</title>
@endsection

@section('breadcrumb')
<ul class="breadcrumb">
    <li><a href="#">Stock</a></li>                    
    <li class="active">Good Request</li>
</ul>
@endsection
        ...
        <form autocomplete="off" id="dataForm" name="dataForm" class="form-horizontal" j-validate>
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3 class="panel-title"><strong>Good Request</strong></h3>
                    <ul class="panel-controls">
                        <li><button type="button" ng-click="add_new()" ng-disabled="add_disable" class="btn btn-primary">Add New</button></li>
    ...
    myApp.controller('mainController', ['$scope', '$http', '$rootScope', 'uiGridConstants', 'uiGridExporterConstants', '$timeout', '$q', '$window', function ($scope, $http, $rootScope, uiGridConstants, uiGridExporterConstants, $timeout, $q, $window) {  
        ...
        $scope.gridOptions = {
            columnDefs: [
                ...
                {
                    field: 'good_request_date_time', 
                    displayName: 'Good Request Date & Time', 
                    cellClass: 'grid-align',
                    width: '20%', 
                    enableCellEdit: false
                },
                ...
                {
                    field: 'good_request_value', 
                    displayName: 'Good Request Value', 
                    cellClass: 'grid-align-right',
                    width: '15%', 
                    footerCellTemplate: '<div class="ui-grid-cell-contents text-right" ><%= grid.appScope.getAggregationGoodRequestTotalValue() | number:2 %></div>',
                    enableCellEdit: false
                },
                {
                    field: 'options', 
        ...
        $scope.getAggregationGoodRequestTotalValue = function(){
            var total_value = 0;
            for(var i=0; i<$scope.gridOptions.data.length; i++){
                total_value += Number($scope.gridOptions.data[i].good_request_value);
            }
            return total_value;
        };
        ...
        $scope.main_refresh = function(){
            ...
            $scope.gridOptions.exporterCsvFilename = 'Good Requests From ' + $scope.data.from + ' To ' + $scope.data.to + '.csv';
            $http({
                method: 'GET',
                url: base_url + '/good_request/good_request_list',
                params: {
                ...
                var data_array = [];
                $.each(response.data.good_requests, function (index, value) {
                    data_array.push({
                        id: value.id,
                        permission: response.data.permission ? 1 : 0,
                        is_posted: value.is_posted,
                        good_request_no: value.good_request_no,
                        good_request_date_time: value.good_request_date_time,
                        remarks: value.remarks,
                        good_request_value: value.good_request_value != 0 ? parseFloat(Math.round(value.good_request_value * 100) / 100).toFixed(2) : ''
                    });
                });    
                $scope.gridOptions.data = data_array;
                $scope.add_disable = response.data.permission || response.data.add_disable;
        ...
        $scope.add_new = function () {
            $window.location.href = base_url + '/good_request/add_new';
        };

        $scope.editRecord = function (row) {
            $window.location.href = base_url + '/good_request/add_new?id='+row.entity.id;
        };
        ...
        $scope.deleteRecord = function (row) {
            $http({
                ...
                swal({
                    title: "Are you sure?",
                    text: "You will not be able to recover <strong>" + response.data.good_request_no + "</strong> good request !",
                    html: true,
                    type: "warning",
                ...
                function(){
                    $http.delete(base_url + '/good_request/'+row.entity.id, row.entity).success(function (response_delete) {
                        swal({
                            title: "Deleted!", 
                            text: response.data.good_request_no + " good request has been deleted.", 
                            type: "success",
