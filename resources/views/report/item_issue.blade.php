...
@section('title')
<title>M3Force | Item Issues</title>
@endsection

@section('breadcrumb')
<ul class="breadcrumb">
    <li><a href="#">Item Issues</a></li>  
</ul>
@endsection
        ...
        <form autocomplete="off" id="dataForm" name="dataForm" class="form-horizontal" j-validate>
            <div class="panel panel-default">
                <div class="panel-heading">
                    <h3 class="panel-title"><strong>Item Issues</strong></h3>
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="col-md-2">
                                <div class="form-group">
                                    ...
                                    <select name="item_issue_type" id="item_issue_type" ng-options="option.name for option in item_issue_type_array track by option.id" ng-model="data.item_issue_type" ng-change="main_refresh()" class="form-control" ></select>
                                </div>
                            </div>
                            <div class="col-md-2">
                                <div class="form-group">
...
<script type="text/javascript">
    ...
    myApp.controller('menuController', function ($scope) {
        ...
        angular.element(document.querySelector('#sub_menu_report_item_issue')).addClass('active');
    });   
    ...
    myApp.controller('mainController', ['$scope', '$http', '$rootScope', 'uiGridConstants', 'uiGridExporterConstants', '$timeout', '$q', '$window', function ($scope, $http, $rootScope, uiGridConstants, uiGridExporterConstants, $timeout, $q, $window) {  
        ...
        $http({
            ...
            $scope.data.item_issue_type = $scope.item_issue_type_array.length > 0 ? $scope.item_issue_type_array[0] : {};
        ...
        $scope.main_refresh = function(){
            ...
            var item_issue_type = $scope.data.item_issue_type && $scope.data.item_issue_type.id != '' ? $scope.data.item_issue_type.name : 'All';
            var item = $scope.data.item ? $scope.data.item.name : 'All';
            $scope.gridOptions.exporterCsvFilename = 'Item Issues From - ' + $scope.data.from + ' To - ' + $scope.data.to + ' Item Issue Type - ' + item_issue_type + ' Item - ' + item + '.csv';
            $http({
                method: 'GET',
                url: base_url + '/report/item_issue_details',
                params: {
                    from: $scope.data.from,                    
                    to: $scope.data.to,                    
                    item_issue_type_id: $scope.data.item_issue_type ? $scope.data.item_issue_type.id : -1,                    
                ...
                $.each(response.data, function (index, value) {
                    data_array.push({
                        id: value.id,
                        item_issue_type: value.item_issue_type,
